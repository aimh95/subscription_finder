...
class U_Net(tf.keras.Model):
    def __init__(self):
        ...
        self.encoder_block1 = EncoderConvBlock(filters=8)
        self.encoder_block2 = EncoderConvBlock(filters=16)
        ...
        self.decoder_block2 = DecoderConvBlock(filters=16)
        self.concat2 = keras.layers.Concatenate(axis=3)
        self.decoder_block3 = DecoderConvBlock(filters=8)

        self.recon1 = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), padding="same", strides=(1, 1), activation="relu")
        self.recon2 = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), padding="same", strides=(1, 1), activation="relu")
