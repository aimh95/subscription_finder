...
def dataset_generator(input_img):
    ...
    fill_color, stroke_color, stroke_width = subscription_meta_gen()
    input_img_pil = tf.keras.utils.array_to_img(input_img[0])
    draw = ImageDraw.Draw(input_img_pil)
    font = ImageFont.truetype(font_selection, font_size)
    for i in range(len(subscriptions)):
        draw.text((x_min, y_min+i*font_size), subscriptions[i], fill="white", font=font, stroke_width=stroke_width, stroke_fill="black")
